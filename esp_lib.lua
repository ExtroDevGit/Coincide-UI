...
local DrawLine = LPHNoVirtualize(function(line, p1, p2, thickness, color)
	...
	line.Position = UDim2.new(
		0,
		math.floor(p1.X + diff.X / 2 - dist / 2 + 0.5),
		0,
...
local FontsToDownload = {
	...
	["Smallest Pixel-7"] = {
		TTF = "https://github.com/ExtroDevGit/Coincide-UI/raw/refs/heads/main/smallest_pixel-7.ttf",
	},
	["Proggy Clean"] = { TTF = "https://github.com/ExtroDevGit/Coincide-UI/raw/refs/heads/main/ProggyClean.ttf" },
	["Tahoma Modern Bold"] = {
		TTF = "https://github.com/ExtroDevGit/Coincide-UI/raw/refs/heads/main/Tahoma-Modern-Bold.ttf",
	},
}
...
local function GetBonePosition(character, boneName)
	...
	elseif boneName == "LeftLowerArm" then
		part = character:FindFirstChild("Left Arm") or character:FindFirstChild("LeftArm")
		if part then
			return (part.CFrame * CFrame.new(0, -0.8, 0)).Position
		end
	elseif boneName == "LeftHand" then
		part = character:FindFirstChild("Left Arm") or character:FindFirstChild("LeftArm")
		if part then
			return (part.CFrame * CFrame.new(0, -1.5, 0)).Position
		end
...
local function AttemptLoadFonts()
	...
	for Name in pairs(FontsToDownload) do
		if not ESPFonts.Loaded[Name] then
...
local CreateESPObj = LPHNoVirtualize(function(name)
	...
	arrowOutline.Size =
		UDim2.new(0, (ESPConfig.OffScreenArrows.Size + 2) * 2, 0, (ESPConfig.OffScreenArrows.Size + 2) * 2)
	...
	espObj.Destroy = function()
		...
		for _, a in pairs(espObj.Adornments) do
			a:Destroy()
		end
		if espObj.ArrowInner then
			espObj.ArrowInner:Destroy()
		end
		if espObj.ArrowOutline then
			espObj.ArrowOutline:Destroy()
		end
		if espObj.ArrowName then
			espObj.ArrowName:Destroy()
		end
		if espObj.ArrowDist then
			espObj.ArrowDist:Destroy()
		end
	end
...
local function BeginConfigCacheFrame()
	...
	table.clear(_overrideCfgCaches)
...
local UpdateESPObj = LPHNoVirtualize(
	function(
		espObj,
		position,
		size,
		...
		if configOverride then
			...
			if not _cfgCache then
				_cfgCache = {}
				_overrideCfgCaches[configOverride] = _cfgCache
			end
		else
			_cfgCache = _defaultCfgCache
		end
		...
		if chamsEnabled and not isDead then
			...
			if chamType == "Highlight" and (instance:IsA("Model") or instance:IsA("BasePart")) then
				...
				h.DepthMode = GetCfg("Chams.Highlight.VisibleCheck") and Enum.HighlightDepthMode.Occluded
			...
			elseif chamType == "Adornment" then
				...
				if visCheck and shouldUpdate then
					...
					local root = instance:IsA("Model")
							and (instance.PrimaryPart or instance:FindFirstChild("HumanoidRootPart") or instance:FindFirstChildWhichIsA(
								"BasePart"
							))
						or instance
			...
			elseif chamType == "MeshChams" then
				...
				if not playerOwner then
					...
					if espObj.MeshHighlight then
						...
						hl.DepthMode = GetCfg("Chams.MeshChams.VisibleCheck") and Enum.HighlightDepthMode.Occluded
							or Enum.HighlightDepthMode.AlwaysOnTop
						hl.Enabled = true
					end
				end
			...
			if rp then
				...
				if not onVp then
					...
					if GetCfg("OffScreenArrows.ArrowMode") == "Compass" then
						...
						local playerRoot = LocalPlayer.Character
							and (
								...
								or LocalPlayer.Character:FindFirstChildWhichIsA("BasePart")
							)
						local fromPos = playerRoot and playerRoot.Position or Camera.CFrame.Position
						local toTarget = (Vector3.new(rp.Position.X, 0, rp.Position.Z) - Vector3.new(
							fromPos.X,
							0,
							fromPos.Z
						)).Unit
					...
					if GetCfg("OffScreenArrows.Names.Enabled") and name and name ~= "" then
						...
						if nFontLoaded then
							espObj.ArrowName.FontFace = nFontLoaded
						end
						...
						ApplyTextOutline(
							espObj.ArrowName,
							nOut and "Full" or "None",
							nOutCol or Color3.fromRGB(0, 0, 0)
						)
					...
					if GetCfg("OffScreenArrows.Distance.Enabled") then
						...
						if dFontLoaded then
							espObj.ArrowDist.FontFace = dFontLoaded
						end
						...
						ApplyTextOutline(
							espObj.ArrowDist,
							dOut and "Full" or "None",
							dOutCol or Color3.fromRGB(0, 0, 0)
						)
		...
		if GetCfg("TextOutline") == false then
		...
		if isCheap then
			...
			for _, l in ipairs(espObj.FlagLabels) do
				l.Visible = false
			end
		...
		if GetCfg("Outlines.Enabled") == false then
			outlineStyle = "None"
		end
		...
		if GetCfg("TeamIndicator.Enabled") and teamOwner and teamOwner.Team then
			local teamColor = GetCfg("TeamIndicator.UseTeamColor") and teamOwner.TeamColor.Color
				or GetCfg("TeamIndicator.Color")
		...
		if teamOwner and GetCfg("FriendlyIndicator.Enabled") then
			if
				GetCfg("FriendlyIndicator.CheckTeam")
				and LocalPlayer.Team ~= nil
				and teamOwner.Team == LocalPlayer.Team
			then
				isFriendly = true
			end
		...
		if isFriendly then
			local friendlyTag = string.format(
				'<font color="%s">%s</font>',
				ColorToHex(GetCfg("FriendlyIndicator.Color")),
		...
		if GetCfg("HealthBar.Enabled") and instance:IsA("Model") and humanoid then
			...
			-- Backward compat: if Enabled is explicitly false, treat as None
			if GetCfg("HealthBar.Outline.Enabled") == false then
				hpOutlineStyle = "None"
			end
			...
			if isHorizontal then
				...
				if hpPos == "Top" then
					espObj.HealthBarOutline.Position = UDim2.new(0, x - 1, 0, y - o - hpSideGap - hpWidth - 1)
				else -- Bottom
				...
				if hpPos == "Left" then
					espObj.HealthBarOutline.Position = UDim2.new(0, x - o - hpSideGap - hpWidth - 1, 0, y - 1)
				else -- Right
			...
			if gradientEnabled and not isHorizontal then
				...
				if followColorText then
					if healthPercent > 0.5 then
						...
						healthColor =
							GetCfg("HealthBar.Gradient.Color1"):Lerp(GetCfg("HealthBar.Gradient.Color2"), ratio)
					else
						local ratio = (0.5 - healthPercent) * 2
						healthColor =
							GetCfg("HealthBar.Gradient.Color2"):Lerp(GetCfg("HealthBar.Gradient.Color3"), ratio)
					end
				end
			else
			...
			if showText then
				...
				if isHorizontal then
					...
					espObj.HealthText.TextXAlignment = hpPos == "Left" and Enum.TextXAlignment.Right
		...
		-- Flags logic
		for _, label in ipairs(espObj.FlagLabels) do
			label.Visible = false
		end
		...
		if GetCfg("Skeleton.Enabled") and instance:IsA("Model") then
			...
			if espObj.Bones then
				for _, b in ipairs(espObj.Bones) do
					b.Visible = false
				end
				for _, b in ipairs(espObj.BoneOutlines) do
					b.Visible = false
				end
			end
		end
...
local Get2DBoundingBox = LPHNoVirtualize(function(instance, trackedData)
	local rootPart
	if instance:IsA("Model") then
		rootPart = instance:FindFirstChild("HumanoidRootPart")
	...
	if not ESPConfig.DynamicBoxes then
		...
		for index = 1, 8 do
			local screenPos = Camera:WorldToViewportPoint(corners[index])
...
local function CheckContains(instance, containsList)
	if type(containsList) ~= "table" or #containsList == 0 then
		return true
	end
	if #containsList == 1 and containsList[1] == "" then
		return true
	end
...
local function CheckNames(instance, namesList)
	if type(namesList) ~= "table" or #namesList == 0 then
		return true
	end
	if #namesList == 1 and namesList[1] == "" then
		return true
	end
...
local ScanDirectories = LPHNoVirtualize(function()
	...
	if ESPConfig.Players then
		for _, player in ipairs(Players:GetPlayers()) do
			if not ESPConfig.LocalPlayer and player == LocalPlayer then
				continue
			end
	...
	for key, config in pairs(ESPConfig.Directories) do
		...
		elseif type(config) == "table" then
			...
			if config.Multiple then
				...
				if
					CheckNames(inst, config.Names)
					and CheckContains(inst, config.Contains)
					and not CheckBlockNames(inst, config.BlockNames)
				then
...
local function RuntimeStep()
	...
	if ESPConfig.LimitFPS and ESPConfig.LimitFPS > 0 then
		if now - lastRender < (1 / ESPConfig.LimitFPS) then
			return
		end
		lastRender = now
	end
	...
	for inst, data in pairs(TrackedInstances) do
		...
		local rootPart = inst:IsA("Model")
				and (inst.PrimaryPart or inst:FindFirstChild("HumanoidRootPart") or inst:FindFirstChildWhichIsA(
					"BasePart"
				))
		...
		if rootPart then
			...
			if not _espDebugDone and onscreen and pos2d and size2d then
				...
				print(
					string.format(
						"[ESP] %-20s | dist=%3dm | box=%4dx%-4d | glowPad=%s",
						data.name or "?",
						dm,
						bw,
						bh,
						gpVisible and tostring(gp) or "hidden"
					)
				)
			end
		else
